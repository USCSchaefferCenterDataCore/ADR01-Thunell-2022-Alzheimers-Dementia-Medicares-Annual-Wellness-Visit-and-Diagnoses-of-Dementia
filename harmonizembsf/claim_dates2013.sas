/* claim_dates2013.sas
   ...
   January 7, 2016: modified for 2013 data
   
   Input files: all claim types for all 2013, i.e., [svc]c[yyyy]
...
%let maxyr=2013;  /* reset maxyr - not updated in setup yet */
...
%resetnseg(ip,2013,2013)
%resetnseg(op,2013,2013)
%resetnseg(snf,2013,2013)
%resetnseg(hha,2013,2013)
%resetnseg(hos,2013,2013)
%resetnseg(car,2013,2013)
%resetnseg(dme,2013,2013)
...
/********************************************************************/
/** Extract dates from 2013 claim files with standard varnames */
...
%procs0210(ip,,,asisnames=&commonstd &staystd &ipsnfstd &wklystd,
           idvar=bene_id,claimvar=clm_id,begy=2013,endy=2013,
           smp=c,clms=);
