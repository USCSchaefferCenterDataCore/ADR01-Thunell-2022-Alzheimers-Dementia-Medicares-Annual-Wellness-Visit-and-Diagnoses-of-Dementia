/* claim_dates2012.sas
   ...
   Input files: all claim types for all 2012, i.e., [svc]c[yyyy]
...
%resetnseg(ip,2012,2012)
%resetnseg(op,2012,2012)
%resetnseg(snf,2012,2012)
%resetnseg(hha,2012,2012)
%resetnseg(hos,2012,2012)
%resetnseg(car,2012,2012)
%resetnseg(dme,2012,2012)
...
/********************************************************************/
/** Extract dates from 2012 claim files with standard varnames */
...
%procs0210(ip,,,asisnames=&commonstd &staystd &ipsnfstd &wklystd,
           idvar=bene_id,claimvar=clm_id,begy=2012,endy=2012,
           smp=c,clms=);
