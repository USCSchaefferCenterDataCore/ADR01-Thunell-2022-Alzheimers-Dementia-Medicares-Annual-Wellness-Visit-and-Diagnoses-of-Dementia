/* claim_dates2014.sas
   ...
   Input files: all claim types for all 2014, i.e., [svc]c[yyyy]
...
%let maxyr=2014;  /* reset maxyr - not updated in setup yet */
...
%resetnseg(ip,2014,2014)
%resetnseg(op,2014,2014)
%resetnseg(snf,2014,2014)
%resetnseg(hha,2014,2014)
%resetnseg(hos,2014,2014)
%resetnseg(car,2014,2014)
%resetnseg(dme,2014,2014)
...
%procs0210(ip,,,asisnames=&commonstd &staystd &ipsnfstd &wklystd,
           idvar=bene_id,claimvar=clm_id,begy=2014,endy=2014,
           smp=c,clms=);
